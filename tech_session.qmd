...
At the time of this writing, the recording is not yet available. This page will be updated with a link to the recording once it is available.
:::
...
CMD ["R", "-e", "options(shiny.port = 8787, shiny.host = '0.0.0.0'); pkgload::load_all(export_all = FALSE,helpers = FALSE,attach_testthat = FALSE);options('golem.app.prod' = TRUE); pilot2wrappers::run_app()"]
```
...
2. The `IMAGE_REGISTRY` parameter defaults to `docker.io`, short for Docker Hub.
3. 
:::